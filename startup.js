...
const startUpMessage = async () => {
  ...
  console.log(
    cliColors.background.blue + "Starting up Blabbit API..." + cliColors.reset
  );
...
const startupTypescript = async () => {
  const distDirExists = fs.existsSync("./dist");
  const removeDistDirCommand = "rm -rf ./dist";
  ...
  try {
    if (distDirExists) {
      const execRemoveDistDirCommand = execSync(removeDistDirCommand);
    }
